...
## Hidden learned-state metadata

Mnemi can store per-question learning state in an HTML comment so it stays invisible in Obsidian preview.

- The metadata is generated by software, not typed by users.
- It should be written directly under the question definition it belongs to.
...
- First value (no key): SHA256 hash of the question text (first 6 characters); used to detect content changes
- `status`: one of `new`, `learning`, `review`, `lapsed`
- `days`: current review interval in days
- `ease`: current ease factor (e.g., 2.4)
- `due`: next due date in `YYYY-MM-DD` format
- `last`: last chosen button (`again`, `hard`, `good`, or `easy`)
- `lapses`: count of failed review attempts
- `reps`: total completed reviews
...
Evaporation
#!
<!-- mnemi: abc123 | status=learning | days=1 | ease=2.3 | due=2026-04-24 | last=again | lapses=2 | reps=5 -->
```

### Parser behavior

- If state metadata is missing, initialize question state as `new`.
- If hash in metadata matches current question hash, use the stored state.
- If hash differs (content changed), reset state to `new`.
- If metadata is malformed, initialize as `new`.
- If multiple state comments are present, use the closest comment immediately following the question.
- On review submission, software rewrites that question's state comment with updated values.
...
- Treat `mnemi:` comments as system-generated metadata and avoid manual edits.
